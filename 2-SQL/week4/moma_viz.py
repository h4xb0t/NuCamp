...
def task1():
    ...
    query = """SELECT department, COUNT(department) FROM moma_works
        GROUP BY department
        ORDER BY COUNT(department) DESC"""  # TODO task 1
...
def task2():
    ...
    query = """SELECT classification, COUNT(*) FROM moma_works
        GROUP BY classification
        HAVING COUNT(*) > 100 ORDER BY COUNT DESC;"""   # TODO task 2
...
def task3():
    ...
    query = """SELECT info -> 'nationality' AS nationality,
...
def task4():
    ...
    query = """SELECT UPPER(info ->> 'gender') AS gender,
    count(info ->> 'gender')
    FROM moma_artists
	WHERE info -> 'gender' IS NOT NULL
    GROUP BY gender
    ORDER BY count(info -> 'gender') DESC;"""  # TODO task 4
...
# SELECT info -> 'gender' AS gender,
#     count(info -> 'gender')
#     FROM moma_artists
# 	WHERE info -> 'gender' IS NOT NULL
#     GROUP BY gender
#     ORDER BY count(info -> 'gender') DESC;

# SELECT g.gender, count(g.gender) FROM
# (SELECT upper(info ->> 'gender') AS gender FROM moma_artists) g
# GROUP BY g.gender
# HAVING g.gender <> 'null';
